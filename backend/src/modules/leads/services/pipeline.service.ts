...
export class PipelineService {
    ...
    async handleContactado(payload: { leadId: number; advisorId: number }) {
        await this.transitionLead(payload.leadId, payload.advisorId, LeadStatus.CONTACTADO);
    ...
    async handleCita(payload: { leadId: number; advisorId: number }) {
        await this.transitionLead(payload.leadId, payload.advisorId, LeadStatus.CITA);
    ...
    async handleCierre(payload: { leadId: number; advisorId: number }) {
        await this.transitionLead(payload.leadId, payload.advisorId, LeadStatus.CIERRE);
    ...
    async handleSeguimiento(payload: { leadId: number; advisorId: number }) {
        await this.transitionLead(payload.leadId, payload.advisorId, LeadStatus.SEGUIMIENTO);
    }

    @OnEvent('command.perdido')
    async handlePerdido(payload: { leadId: number; advisorId: number }) {
        await this.transitionLead(payload.leadId, payload.advisorId, LeadStatus.PERDIDO);
    }

    @OnEvent('command.intento_contacto')
    async handleIntento(payload: { leadId: number; advisorId: number }) {
    ...
    async handleAssign(payload: { leadId: number; advisorId: number }) {
        await this.transitionLead(payload.leadId, payload.advisorId, LeadStatus.ASIGNADO);
        // Start SLA
        await this.slaService.createSla(payload.leadId, payload.advisorId);
    }

    private async transitionLead(leadId: number, advisorId: number, nextStatus: LeadStatus) {
